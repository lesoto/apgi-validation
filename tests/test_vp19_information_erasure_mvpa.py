...
class TestStimulusPairEEGSimulator:
    ...
    def test_prototypes_are_unit_vectors(self):
        ...
        assert abs(np.linalg.norm(sim._face_proto) - 1.0) < 1e-9
        assert abs(np.linalg.norm(sim._house_proto) - 1.0) < 1e-9

    def test_prototypes_differ(self):
        sim = StimulusPairEEGSimulator()
        assert not np.allclose(sim._face_proto, sim._house_proto)

    def test_epoch_features_shape(self):
        sim = StimulusPairEEGSimulator()
        amp = np.ones(N_BINS)
        feats = sim._epoch_features("faces", amp)
        assert feats.shape == (N_BINS, N_SENSORS)
    ...
    def test_ignition_profile_winner_higher_than_loser_post_ignition(self):
        ...
        assert winner_profile[post_mask].mean() > loser_profile[post_mask].mean()
    ...
    def test_simulate_dataset_feature_shapes(self):
        ...
        for _, losing_feats, winning_feats in dataset:
            assert losing_feats.shape == (N_BINS, N_SENSORS)
            assert winning_feats.shape == (N_BINS, N_SENSORS)
    ...
    def test_reported_and_losing_are_complementary(self):
        ...
        for meta, _, _ in dataset:
            assert meta.reported_category in CATEGORIES
            assert meta.losing_category in CATEGORIES
            assert meta.reported_category != meta.losing_category
    ...
    def test_ignition_strength_in_range(self):
        ...
        dataset = sim.simulate_dataset()
        for meta, _, _ in dataset:
            assert 0.0 <= meta.ignition_strength <= 1.0
...
class TestTimeResolvedMVPADecoder:
    ...
    def _make_decodable_data(
        self, n_trials: int = 20, rng: Optional[np.random.Generator] = None
    ):
    ...
    def test_decode_timeseries_shape(self):
        ...
        assert acc_ts.shape == (N_BINS,)
        assert isinstance(perm_thresh, float)
...
class TestInformationErasureValidator:
    ...
    def test_load_idempotent(self):
        ...
        n = len(v.dataset)
        v.load_or_generate_data()
        assert len(v.dataset) == n

    def test_build_feature_matrices_shapes(self, fast_validator):
        ig_f, ig_l, no_lf, no_wf = fast_validator._build_feature_matrices()
        assert ig_f.ndim == 3
        assert ig_f.shape[1] == N_BINS
        assert ig_f.shape[2] == N_SENSORS
        assert len(ig_l) == len(ig_f)

    def test_ig_accuracy_ts_shape(self, fast_validator):
        assert fast_validator._ig_accuracy_ts is not None
        assert fast_validator._ig_accuracy_ts.shape == (N_BINS,)

    def test_no_ig_accuracy_ts_shapes(self, fast_validator):
        assert fast_validator._no_ig_losing_accuracy_ts is not None
        assert fast_validator._no_ig_winning_accuracy_ts is not None
        assert fast_validator._no_ig_losing_accuracy_ts.shape == (N_BINS,)
        assert fast_validator._no_ig_winning_accuracy_ts.shape == (N_BINS,)
    ...
    def test_run_full_validation_structure(self, fast_validator):
        ...
        assert results["protocol_id"] == "VP_19_InformationErasure_MVPA"
    ...
    def test_v19_2_consecutive_bins_counter(self):
        ...
        assert result["max_consecutive_below_chance_bins"] >= 4
...
class TestRunValidation:
    ...
    def test_returns_dict(self):
        result = run_validation(seed=0, n_permutations=FAST_N_PERM)
        assert isinstance(result, dict)

    def test_has_protocol_id(self):
        result = run_validation(seed=1, n_permutations=FAST_N_PERM)
        assert result["protocol_id"] == "VP_19_InformationErasure_MVPA"
    ...
    def test_deterministic(self):
        ...
        assert abs(r1["overall_score"] - r2["overall_score"]) < 1e-9
