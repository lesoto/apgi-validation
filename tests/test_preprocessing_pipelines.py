...
class TestPreprocessingConfig:
    ...
    def test_default_config(self):
        ...
        assert config.eeg_bandpass_high == 40.0, "Default EEG high cutoff should be 40 Hz"
        ...
        assert config.eeg_artifact_threshold == 5.0, "Default artifact threshold should be 5.0"

        # Pupil preprocessing defaults
        assert config.pupil_blink_detection, "Blink detection should be enabled by default"
        assert config.pupil_interpolation_method == "linear", "Default interpolation should be linear"
        assert config.pupil_smoothing_window == 5, "Default smoothing window should be 5"

        # EDA preprocessing defaults
        assert config.eda_lowpass_cutoff == 5.0, "Default EDA cutoff should be 5 Hz"
        assert config.eda_smoothing_window == 10, "Default EDA smoothing window should be 10"

        # Heart rate preprocessing defaults
        assert config.hr_outlier_threshold == 3.0, "Default HR outlier threshold should be 3.0"
        assert config.hr_interpolation_method == "cubic", "Default HR interpolation should be cubic"

        # General preprocessing defaults
        assert config.missing_data_strategy == "interpolate", "Default missing strategy should be interpolate"
        ...
        assert config.outlier_threshold == 1.5, "Default outlier threshold should be 1.5"
        assert config.normalization_method == "zscore", "Default normalization should be zscore"

        # Resampling defaults
        assert config.target_sampling_rate == 250.0, "Default target rate should be 250 Hz"
        assert config.resample_method == "interpolate", "Default resampling should be interpolate"
    ...
    def test_custom_config(self):
        ...
        assert config.eeg_artifact_threshold == 3.0, "Should use custom artifact threshold"
        ...
        assert config.normalization_method == "minmax", "Should use custom normalization"
...
class TestEEGPreprocessor:
    ...
    def test_eeg_preprocessor_initialization(self):
        ...
        assert isinstance(preprocessor.preprocessing_log, list), "Should initialize log list"
    ...
    def test_preprocess_eeg_no_eeg_columns(self):
        ...
        result = preprocessor.preprocess_eeg(df, sampling_rate=1000.0, show_progress=False)
...
class TestPupilPreprocessor:
    ...
    def test_detect_blinks(self, sample_pupil_data):
        ...
        assert "pupil_diameter_blink" in result.columns, "Should add blink detection column"
        assert result["pupil_diameter_blink"].dtype == bool, "Blink column should be boolean"
    ...
    def test_interpolate_blinks(self, sample_pupil_data):
        ...
        # First detect blinks
        df_with_blinks = preprocessor._detect_blinks(sample_pupil_data, "pupil_diameter")
...
class TestEDAPreprocessor:
    ...
    def test_estimate_sampling_rate_eda(self, sample_eda_data):
        ...
        assert isinstance(estimated_fs, (float, type(None))), "Should return float or None"
...
class TestErrorHandling:
    ...
    def test_empty_dataframe(self):
        ...
        eeg_preprocessor.preprocess_eeg(df, sampling_rate=1000.0, show_progress=False)

        # Pupil preprocessing
        pupil_preprocessor = PupilPreprocessor(config)
        pupil_preprocessor.preprocess_pupil(df, show_progress=False)
    ...
    def test_zero_sampling_rate(self):
        ...
        preprocessor._apply_bandpass_filter(df["eeg_1"], 0.0)

    def test_invalid_frequency_bands(self):
        """Test with invalid frequency bands."""
        config = PreprocessingConfig(eeg_bandpass_low=50.0, eeg_bandpass_high=10.0)  # Low > High
        preprocessor = EEGPreprocessor(config)

        df = pd.DataFrame({"eeg_1": [1.0, 2.0, 3.0]})
        preprocessor._apply_bandpass_filter(df["eeg_1"], 1000.0)
