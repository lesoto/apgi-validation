...
def wrapped_somatic_marker_dynamics(
    M: float,
    eps_i: float,
    beta_M: float,
    gamma_context: float = 0.0,
    C: float = 0.0,
    tau_M: float = 1.0,
    sigma_M: float = 0.1,
    dt: float = 0.1,
...
class TestCoreIgnitionSystem:
    ...
    def test_accumulated_signal_basic(self):
        ...
        # The actual implementation includes noise and modulation terms
        # Just verify it's a reasonable float value and not NaN/inf
...
class TestDynamicalSystemEquations:
    ...
    def test_signal_dynamics_negative_change(self):
        ...
        S_new = wrapped_signal_dynamics(S=1.0, Pi_e=-0.1, eps_e=0.05)
        # dS/dt = -τ_S⁻¹S(t) + ½Π^e(t)(ε^e(t))² + ½Π^i_eff(t)(ε^i(t))² + σ_Sξ_S(t)
        # With default values: additional terms contribute to result
    ...
    def test_signal_dynamics_zero_surprise(self):
        ...
        # dS/dt = -τ_S⁻¹S(t) + ½Π^e(t)(ε^e(t))² + ½Π^i_eff(t)(ε^i(t))² + σ_Sξ_S(t)
        # With eps_e=0, precision term drops out, but other terms remain
    ...
    def test_threshold_dynamics_clipping(self):
        ...
        assert theta_high == 2.0  # Should be clipped to max
        ...
        assert theta_low == 2.0  # Should be clipped to min
    ...
    def test_somatic_marker_dynamics_clipping(self):
        ...
        # Test upper bound
        M_high = wrapped_somatic_marker_dynamics(M=5.0, eps_i=0.5, beta_M=2.0)
        assert M_high == 2.0  # Should be clipped to max

        # Test lower bound
        M_low = wrapped_somatic_marker_dynamics(M=0.5, eps_i=0.5, beta_M=2.0)
    ...
    def test_arousal_dynamics_clipping(self):
        ...
        # Test upper bound
        A_high = wrapped_arousal_dynamics(
            A=2.0, A_target=0.8, tau_A=0.5, sigma_A=0.1, dt=0.1
        )
        assert A_high == 1.0  # Should be clipped to max

        # Test lower bound
        A_low = wrapped_arousal_dynamics(
...
class TestDerivedQuantities:
    ...
    def test_latency_to_ignition_negative_latency(self):
        ...
        ignition_time = wrapped_latency_to_ignition(S_0=1.0, theta=3.0, I=1.0)
    ...
    def test_boundary_condition_handling(self):
        ...
        theta_at_min = wrapped_threshold_dynamics(
        ...
        theta_at_alert = wrapped_threshold_dynamics(
...
class TestAPGIParameters:
    ...
    def test_precision_expectation_gap_computation(self):
        ...
        params = APGIParameters()
        params.Pi_e_actual = 0.8
        params.Pi_i_actual = 0.6
        params.Pi_e_baseline = 0.5
        params.Pi_i_baseline = 0.4
