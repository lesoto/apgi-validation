...
class APGILogger:
    ...
    def export_logs(
        ...
        try:
            ...
            if format_type.lower() == "json":
                ...
                with open(output_path, "w", newline="", encoding="utf-8") as f:
                    ...
                    for entry in log_entries:
                        writer.writerow(
                            {k: v for k, v in entry.items() if k in fieldnames}
                        )
                ...
                with open(output_path, "w", encoding="utf-8") as f:
                    ...
                    f.write(
                        f"Time range: {log_entries[0]['timestamp']} to {log_entries[-1]['timestamp']}\n"
                    )
                    f.write("=" * 80 + "\n\n")

                    for entry in log_entries:
                        f.write(
                            f"{entry['timestamp']} | {entry['level']:8} | {entry['location']} | {entry['message']}\n"
                        )
            ...
            logger.info(
                f"Successfully exported {len(log_entries)} log entries to {output_file}"
            )
            return True
