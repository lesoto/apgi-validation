...
class SurpriseIgnitionSystem:
    ...
    def step(self, dt, inputs):
        ...
        dTheta = ((self.p['theta_0'] - self.theta) / self.p['tau_theta'] + modulation) * dt + self.p['sigma_theta'] * xi_theta
...
def run_simulation():
    ...
    ax0.plot(time, history['theta'], label=r'$\theta_t$ (Threshold)', color='orange', linestyle='--', linewidth=1.5)
    ...
    if len(ignition_times) > 0:
        ax0.scatter(ignition_times, ignition_vals, color='red', s=50, zorder=5, label='Ignition ($B_t=1$)')
    ...
    ax1 = axes[1]
    ax1.plot(time, history['input_drive'], color='green', alpha=0.6, label='Total Input Drive')
