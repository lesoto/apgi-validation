...
class NeuralSignalGenerator:
    ...
    def generate_hep_waveform(Pi_i, sampling_rate=1000, duration=0.6):
        ...
        hep_waveform = hep_amplitude * np.exp(
    ...
    def generate_p3b_waveform(amplitude_factor, sampling_rate=1000, duration=0.8):
        ...
        p3b_waveform = p3b_amplitude * np.exp(
            -((time_ms - peak_time) ** 2) / (2 * sigma**2)
        )
    ...
    def generate_pupil_response(
        Pi_i, sampling_rate=1000, duration=3.0, contamination_rate=0.15
    ):
        ...
        pupil_response = dilation_magnitude * np.exp(
            -((time_s - peak_time) ** 2) / (2 * sigma**2)
        )
    ...
    def _pink_noise(n_samples, amplitude):
        ...
        for octave in range(5):
            step = 2**octave
            pink += np.repeat(np.random.randn(n_samples // step + 1), step)[
                :n_samples
            ] / (octave + 1)
...
class APGIDynamics:
    ...
    def simulate_surprise_accumulation(
        epsilon_e, epsilon_i, Pi_e, Pi_i, beta, tau=0.2, dt=0.001, duration=1.0
    ):
        """
...
def build_apgi_model(subjects):
    ...
    p3b_ratio_obs = np.array(
        [s["p3b_intero_mean"] / (s["p3b_extero_mean"] + 0.01) for s in subjects]
    )
    ...
    with pm.Model() as model:
        ...
        logit_p = alpha_constrained[:, None] * (
            intensity_grid - theta0_constrained[:, None]
        )
...
def assess_predictive_validity(subjects, trace, test_fraction=0.2, seed=42):
    ...
    theta0_est = (
        trace.posterior["theta0"].mean(dim=["chain", "draw"]).values[test_indices]
    )
...
def assess_test_retest_reliability(n_subjects=40, seed=42):
    ...
    for param in param_names:
        ...
        between_subj_var = np.var(
            [np.mean([vals_t1[i], vals_t2[i]]) for i in range(n_subjects)]
        )
        within_subj_var = np.mean(
            [np.var([vals_t1[i], vals_t2[i]]) for i in range(n_subjects)]
        )
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print(
        f"{'Parameter':<12} {'r':<8} {'RMSE':<10} {'Coverage':<10} {'Threshold':<10} {'Status':<8}"
    )
    print("-" * 80)
    ...
    for param, res in recovery_results.items():
        ...
        print(
            f"{param:<12} {res['r']:>7.3f}  {res['rmse']:>9.4f}  "
    ...
    if falsified:
        ...
        for param, res in recovery_results.items():
            if not res["passes"]:
                print(
                    f"  - {param}: r = {res['r']:.3f} (threshold: {res['threshold']:.2f})"
                )
    else:
    ...
    print("-" * 80)
    print(
        f"{'Emotional Interference (Stroop RT)':<40} {pred_validity['emotional_interference']['r']:>7.3f}  "
        f"{pred_validity['emotional_interference']['r2']:>7.3f}"
    )
    print(
        f"{'CPT Attentional Lapses':<40} {pred_validity['cpt_lapses']['r']:>7.3f}  "
        f"{pred_validity['cpt_lapses']['r2']:>7.3f}"
    )
    print(
        f"{'Body Vigilance Scale':<40} {pred_validity['body_vigilance']['r']:>7.3f}  "
    ...
    for i, (param, label) in enumerate(zip(params, labels)):
        ...
        lims = [
            min(true_vals.min(), recovered_vals.min()) * 0.95,
    ...
    print(
        f"Parameter Recovery: {recovery_score}/50 ({passed_params}/4 parameters passed)"
    )
    ...
    print(
        f"Test-Retest Reliability: {reliability_score}/15 (mean ICC = {np.mean(icc_vals):.3f})"
    )
