    ...
    for session in range(n_sessions):
        ...
        for subj_id in range(n_subjects):
            ...
            for trial in range(n_trials):
                ...
                seen_prob = 1 / (
                    1 + np.exp(-alpha * (current_intensity + perceptual_noise - theta0))
                )
                ...
                conf = min(
                    ...
                    max(
                        1,
                        int(
                            3 * abs(current_intensity - theta0)
                            + 1
            ...
            for trial in range(n_hb_trials):
                ...
                pupil = (0.3 * Pi_i_approx + pupil_noise) * np.random.uniform(
                    0.8, 1.2
                )  # Blink variability
            ...
            session_data[subj_id] = {
                ...
                "oddball": {
                    ...
                    "ratio": (
                        ...
                        if np.mean(p3b_extero) > 0
                        else 1.0
                    ),
                },
                "neural_baseline": {"rest_p3b": rest_p3b, "alpha_power": alpha_power},
                "true_params": {
                    "theta0": theta0,
                    "alpha": alpha,
                    "tau": tau,
...
def build_apgi_model(data, estimate_tau=True):
    ...
    with pm.Model(
        ...
        beta_Pi_i = pm.Deterministic(
            "beta_Pi_i", pm.math.exp(mu_beta_Pi_i + beta_Pi_i_offset * sigma_beta_Pi_i)
        )
        ...
        pm.Normal("p3b_ratio", mu=expected_ratio, sigma=0.25, observed=ratios)
...
def assess_predictive_validity(data, trace, n_simulations=1000):
    ...
    for param in ["theta0", "alpha", "beta_Pi_i", "Pi_e0", "sigma"]:
        if param in trace.posterior:
            param_ests[param] = (
                trace.posterior[param].mean(dim=["chain", "draw"]).values
            )
    ...
    pred_rt = (
        450
        + 50 * param_ests.get("beta_Pi_i", np.zeros(n_subjects))
    ...
    cpt_lapses = np.random.poisson(5, n_subjects) + 10 * param_ests.get(
        "theta0", np.zeros(n_subjects)
    )
    cpt_variability = np.random.normal(100, 20, n_subjects) + 30 * param_ests.get(
        "theta0", np.zeros(n_subjects)
    )
    ...
    bvs_scores = np.random.normal(20, 6, n_subjects) + 5 * param_ests.get(
        "beta_Pi_i", np.zeros(n_subjects)
    )
    ...
    if "theta0" in trace.posterior and "alpha" in trace.posterior:
        ...
        theta0_samples = np.random.choice(
            trace.posterior["theta0"].values.flatten(), n_samples
        )
        alpha_samples = np.random.choice(
            trace.posterior["alpha"].values.flatten(), n_samples
        )
        ...
        for i in range(n_samples):
            prob = 1 / (
                1 + np.exp(-alpha_samples[i] * (intensities - theta0_samples[i]))
            )
    ...
    return {
        "emotional_interference": {"r": r_rt, "r2": r2_rt, "cv_r2": cv_r2, "p": p_rt},
        "cpt_performance": {
            "lapses_r": r_lapses,
            "lapses_r2": r2_lapses,
            "p": p_lapses,
        },
        "body_vigilance": {"r": r_bvs, "p": p_bvs},
        "psychometric_prediction": {"r2": pred_r2},
...
if __name__ == "__main__":
    ...
    recovery_results, falsified, failures = validate_parameter_recovery(
        true_params, trace1
    )
    ...
    print("=" * 70)
    print(
        f"{'Parameter':<12} {'r':<8} {'RMSE':<8} {'Coverage':<10} {'True Mean':<12} {'Rec Mean':<12}"
    )
    print("-" * 70)

    for param, res in recovery_results.items():
        print(
            f"{param:<12} {res['r']:.3f}    {res['rmse']:.3f}     {res['coverage']:.3f}       "
    ...
    if icc_failures:
        ...
        falsified = True
        failures.extend(
            [f"{p}: ICC = {reliability[p]['ICC']:.3f}" for p in icc_failures]
        )
    ...
    print(
        f"   Cross-validated R² = {pred_validity['emotional_interference']['cv_r2']:.3f}"
    )

    print("\n2. Continuous Performance Task:")
    print(
        f"   Lapses correlation: r = {pred_validity['cpt_performance']['lapses_r']:.3f}"
    )
    ...
    if pred_validity["psychometric_prediction"]["r2"] < 0.45:
        ...
        falsified = True
        failures.append(
            f"Predictive R² = {pred_validity['psychometric_prediction']['r2']:.3f}"
        )
    ...
    print(
        f"Parameters with ESS < 400: {[(var, float(val)) for var, val in ess.items() if val.min() < 400]}"
    )
    ...
    print(
        f"Parameters with R-hat > 1.01: {[(var, float(val)) for var, val in rhat.items() if val.max() > 1.01]}"
    )
    ...
    for idx, param in enumerate(params_to_plot):
        if param in recovery_results:
            ...
            rec_vals = (
                recovery_results[param]["rec_mean"]
                if "rec_mean" in recovery_results[param]
                else trace1.posterior[param].mean(dim=["chain", "draw"]).values[:100]
            )
