...
This document maps APGI framework controls to formal compliance standards including ISO 27001, SOC 2, and NIST CSF.
...
- `utils/security_logging_integration.py` implements SecurityContext
- Status: Implemented
...
- Default SecurityContext has empty roles set
- Status: Implemented
...
- `utils/logging_config.py` implements JsonFormatter
- Status: Implemented
...
- Data protection workflows in `utils/data_protection.py`
- Status: Partial - needs classification schema
...
- Path security validation in `utils/path_security.py`
- TOCTOU mitigation
- Status: Partial - needs hardening checklist
...
- `utils/dependency_scanner.py` for dependency analysis
...
- `utils/security_logging_integration.py` - Authorization middleware
- `utils/security_audit_logger.py` - Audit logging
- `utils/path_security.py` - Path validation
...
- `utils/cache_manager.py` - Cache SLO monitoring
