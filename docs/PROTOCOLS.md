...
All F5 thresholds in falsification_thresholds.py audited and corrected to match inline "VALIDATED" constants in Falsification-ActiveInferenceAgents-F1F2.py. Constants now use paper-cited values: F5_4_MIN_PEAK_SEPARATION = 3.0, F5_1_MIN_PROPORTION = 0.75, F5_1_MIN_ALPHA = 4.0, F5_5_PCA_MIN_VARIANCE = 0.70, F5_5_MIN_LOADING = 0.60.
...
Cultural neuroscience motivation documented in lines 6-32 of Validation-Protocol-11.py, mapping to APGI parameters {θ₀, Πⁱ, β, α} with quantitative predictions from APGI-MULTI-SCALE-CONSCIOUSNESS-Paper.
...
Framework-Level Aggregator (APGI-Falsification-Aggregator.py) has complete implementation:
...
Framework-level criteria (conditions a and b, the 14 named predictions) live in APGI-Falsification-Aggregator.py.
Naming/routing collision fixed: Protocol 3 path corrected to "../Validation/ActiveInference-AgentSimulations-Protocol3.py".
