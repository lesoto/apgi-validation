...
class EvolutionaryAPGIEmergence:
    ...
    def run_evolution(self, max_time_seconds: float = 30.0) -> Dict:
        ...
        for generation in range(self.n_generations):
            ...
            history['best_genome'].append(
                population[np.argmax(fitness_scores)].copy()
            )
            ...
            arch_freq = {
                ...
                'has_precision_weighting': np.mean([g['has_precision_weighting'] for g in population])
            }
            ...
            if generation % 10 == 0:  # More frequent updates
                ...
                print(f"Gen {generation} ({elapsed:.1f}s): Best={np.max(fitness_scores):.3f}, "
    ...
    def analyze_emergence(self, history: Dict) -> Dict:
        ...
        apgi_emerged = (
            final_freq['has_threshold'] > 0.9 and
            final_freq['has_intero_weighting'] > 0.9 and
            final_freq['has_somatic_markers'] > 0.9
        )
        
        # Compute selection coefficients
        # (how quickly did each trait spread?)
        selection_coefficients = {}
        for trait in ['has_threshold', 'has_intero_weighting', 
                     'has_somatic_markers', 'has_precision_weighting']:
