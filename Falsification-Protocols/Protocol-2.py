...
class IowaGamblingTaskEnvironment:
    ...
    def __init__(self, n_trials: int = 100):
        ...
        self.decks = {
            'A': {'reward_mean': 100, 'reward_std': 50, 
                  'loss_prob': 0.5, 'loss_mean': 250,
                  'intero_cost': 0.8},
            'B': {'reward_mean': 100, 'reward_std': 50,
                  'loss_prob': 0.1, 'loss_mean': 1250,
                  'intero_cost': 0.5},
            'C': {'reward_mean': 50, 'reward_std': 25,
                  'loss_prob': 0.5, 'loss_mean': 50,
                  'intero_cost': 0.1},
            'D': {'reward_mean': 50, 'reward_std': 25,
                  'loss_prob': 0.1, 'loss_mean': 250,
                  'intero_cost': 0.05}
        }
    ...
    def reset(self) -> Dict:
        ...
        return {
            'extero': np.zeros(32),
            'intero': self._generate_intero_signal(0.1)
        }
...
class ThreatRewardTradeoffEnvironment:
    ...
    def step(self, action: int) -> Tuple[float, float, Dict, bool]:
        ...
        # Threat response
        threat = opt['threat']
        self.threat_accumulator = (self.threat_decay * self.threat_accumulator + 
                                   threat)
