...
class APGIVisualizer:
    ...
    def plot_ignition_landscape(self,
        ...
        p1_range = np.linspace(self.df[parameter1].min() * 0.8, 
                              self.df[parameter1].max() * 1.2, 
                              resolution)
        p2_range = np.linspace(self.df[parameter2].min() * 0.8,
                              self.df[parameter2].max() * 1.2,
                              resolution)
    ...
    def plot_transition_pathways(self,
                               from_state: str,
                               to_state: str,
        ...
        dimensions = []
        param_names = ['Pi_e', 'Pi_i_eff', 'theta_t', 'M_ca', 'S_t', 'ignition_probability']
        ...
        colors = [self.categories.get(state, StateCategory.UNELABORATED).color 
                 for state in pathway]
    ...
    def plot_state_radar(self,
        ...
        fig.update_layout(
            polar=dict(
                radialaxis=dict(
                    visible=True,
                    range=[0, 1.1] if normalize else None
                )
            ),
            showlegend=True,
            legend=dict(
    ...
    def create_state_summary_dashboard(self,
                                     state_name: str,
                                     save_path: Optional[str] = None) -> Optional[go.Figure]:
        """
        ...
        if category_states:
            ...
            for i, (comp_state, values) in enumerate(zip([state_name] + category_states, radar_data)):
                fig.add_trace(
                    go.Scatterpolar(
                        r=values,
    ...
    def export_visualization_report(self, 
                                  output_dir: str = "./apgi_visualizations") -> Dict[str, str]:
        """
...
class APGIVisualizerGUI:
    ...
    def setup_gui(self):
        ...
        # Visualization Type
        ttk.Label(control_frame, text="Visualization Type:").grid(row=0, column=0, sticky=tk.W, pady=5)
        ...
        self.state_combo = ttk.Combobox(control_frame, textvariable=self.state_var, state="readonly")
        self.state_combo.grid(row=3, column=0, sticky=(tk.W, tk.E), pady=(0, 10))
        
        # Multiple States for Radar
        ttk.Label(control_frame, text="Multiple States (Radar):").grid(row=4, column=0, sticky=tk.W, pady=5)
        ...
        self.from_state_combo = ttk.Combobox(control_frame, textvariable=self.from_state_var, state="readonly")
        ...
        self.to_state_combo = ttk.Combobox(control_frame, textvariable=self.to_state_var, state="readonly")
        self.to_state_combo.grid(row=9, column=0, sticky=(tk.W, tk.E), pady=(0, 10))
        
        # Buttons
        ttk.Button(control_frame, text="Generate Visualization", 
                  command=self.generate_visualization).grid(row=10, column=0, sticky=(tk.W, tk.E), pady=5)
        ttk.Button(control_frame, text="Save Visualization", 
                  command=self.save_visualization).grid(row=11, column=0, sticky=(tk.W, tk.E), pady=5)
        ttk.Button(control_frame, text="Generate Full Report", 
                  command=self.generate_full_report).grid(row=12, column=0, sticky=(tk.W, tk.E), pady=5)
        ...
        def generate():
            try:
                ...
                if report_dir:
                    ...
                    self.root.after(0, lambda: self.status_var.set(f"Report generated with {len(report_files)} files"))
...
def compare_states(state1: str, state2: str) -> Dict[str, Tuple[float, float, float]]:
    ...
    params = ['Pi_e', 'Pi_i_baseline', 'Pi_i_eff', 'theta_t', 'S_t', 'M_ca', 'beta', 'z_e', 'z_i']
...
def generate_state_comparison_table(states: List[str]) -> str:
    ...
    headers = ['State', 'Π_e', 'Π_i_eff', 'θ_t', 'S_t', 'M_ca', 'β', 'z_e', 'z_i', 'P(ign)']
    ...
    lines.append("│" + "│".join(headers[i].center(col_widths[i]) for i in range(len(headers))) + "│")
    lines.append("├" + "┼".join("─" * w for w in col_widths) + "┤")
    
    for row in rows:
        lines.append("│" + "│".join(str(row[i]).center(col_widths[i]) for i in range(len(row))) + "│")
...
def run_enhanced_demo():
    ...
    if has_viz:
        ...
        print("-" * 40)
        visualizer = APGIVisualizer(PSYCHOLOGICAL_STATES, STATE_CATEGORIES)
        print(f"   Loaded {len(PSYCHOLOGICAL_STATES)} states across {len(set(STATE_CATEGORIES.values()))} categories")
    ...
    print(generate_state_comparison_table([
    ...
    test_params = create_apgi_params(
        Pi_e=7.0, Pi_i_baseline=3.0, M_ca=1.6, beta=0.7,
        z_e=2.2, z_i=1.8, theta_t=-2.0
    )
    ...
    print(f"θ_t range: {min(all_theta):+.1f} - {max(all_theta):+.1f} (mean: {np.mean(all_theta):+.2f})")
