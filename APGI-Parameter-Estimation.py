...
class NeuralSignalGenerator:
    ...
    def _pink_noise(n_samples, amplitude):
        ...
        for octave in range(5):
            step = 2**octave
            pink += np.repeat(np.random.randn(n_samples // step + 1), step)[:n_samples] / (
                octave + 1
            )
...
def build_apgi_model(subjects):
    ...
    # n_levels = len(subjects[0]["intensities"])  # Commented out - unused
...
def assess_test_retest_reliability(n_subjects=40, seed=42):
    ...
    for param in param_names:
        ...
        between_subj_var = np.var([np.mean([vals_t1[i], vals_t2[i]]) for i in range(n_subjects)])
        within_subj_var = np.mean([np.var([vals_t1[i], vals_t2[i]]) for i in range(n_subjects)])
...
if __name__ == "__main__":
    ...
    print("  - Detection: 8 intensity levels × 25 trials per subject")
    print("  - Heartbeat: 60 trials per subject with HEP/pupil signals")
    print("  - Oddball: 30 intero + 30 extero deviants per subject")
