...
class APGINormalizer:
    ...
    def fit(self, normative_data: Dict[str, np.ndarray], 
        ...
        for var_name, data in normative_data.items():
            ...
            if len(transformed_data) < 5000:
                ...
                if p_value < 0.05 and var_name not in self.transforms:
                    warnings.warn(f"{var_name} is non-Gaussian (p={p_value:.4f}). Consider adding a transformation.")
    
    def transform(self, raw_measurements: Dict[str, float], 
                  use_session_stats: bool = False,
                  session_stats: Optional[Dict[str, Dict]] = None) -> Dict[str, float]:
        """
    ...
    @classmethod
    def load(cls, filepath: str, transforms: Optional[Dict[str, Callable]] = None,
             use_robust_stats: bool = False):
...
class APGICoreIntegration:
    ...
    def compute_running_precision(self,
                                 new_value: float,
                                 modality_type: str,
    ...
    def compute_somatic_modulation(self,
                                  Pi_i_baseline: float,
                                  M_ca: float,
                                  beta: float) -> float:
        """
    ...
    def compute_accumulated_signal(self,
                                  z_e: float,
                                  z_i: float,
                                  Pi_e: float,
                                  Pi_i_eff: float) -> float:
        """
    ...
    def compute_ignition_probability(self,
                                    S_t: float,
                                    theta_t: float) -> float:
        """
    ...
    def integrate_multimodal_zscores(self,
                                    z_scores: Dict[str, float],
                                    raw_signals: Dict[str, np.ndarray],
                                    M_ca: Optional[float] = None,
                                    beta: Optional[float] = None) -> APGIParameters:
        """
        ...
        z_extero_list = [z_scores[m] for m in self.EXTEROCEPTIVE_MODALITIES 
                        if m in z_scores]
        z_intero_list = [z_scores[m] for m in self.INTEROCEPTIVE_MODALITIES 
        ...
        extero_signals = [raw_signals[m] for m in self.EXTEROCEPTIVE_MODALITIES 
                         if m in raw_signals]
        intero_signals = [raw_signals[m] for m in self.INTEROCEPTIVE_MODALITIES 
...
def compare_implementations():
    ...
    incorrect_signal = (
        incorrect_params['Π_e'] +  # Actually adding z-scores
        incorrect_params['Π_i']
    )
    ...
    print(f"         Z-scores measure MAGNITUDE, precision measures RELIABILITY")
    ...
    print(f"\nAccumulated Signal:")
    print(f"  S_t = Π_e·|z_e| + Π_i_eff·|z_i|")
    print(f"  S_t = {params.S_t:.3f}")
    
    print(f"\nKey Differences:")
    print(f"  1. Precision (Π) computed as 1/variance, NOT copied from z-score")
    print(f"  2. Somatic modulation applied: Π_i_eff = Π_i × exp(β·M)")
    print(f"  3. Formula uses multiplication: Π·|z|, not addition")
    print(f"  4. Separate magnitude (z) and reliability (Π) terms")
...
class APGIArtifactRejection:
    ...
    def detect_eeg_artifacts(self, eeg: np.ndarray, fs: int = 250) -> np.ndarray:
        ...
        clean_mask = ~(amplitude_artifacts | gradient_artifacts | 
                       noise_artifacts | flatline_artifacts)
    ...
    def interpolate_artifacts(self, data: np.ndarray, clean_mask: np.ndarray,
        ...
        if method == 'cubic':
            ...
            interpolator = lambda x: np.interp(x, clean_indices, data[clean_mask])
...
class APGISpectralAnalysis:
    ...
    def compute_band_power(self, eeg: np.ndarray, band: str = 'gamma_low',
                          channels: Optional[List[int]] = None) -> float:
        """
    ...
    def _multitaper_psd(self, eeg: np.ndarray, freq_range: Tuple[float, float]) -> np.ndarray:
        ...
        tapers = windows.dpss(n_samples, nw, n_tapers)
    ...
    def compute_relative_power(self, eeg: np.ndarray, target_band: str = 'gamma_low') -> float:
        ...
        total_power = sum([
            self.compute_band_power(eeg, band) 
            for band in self.BANDS.keys()
        ])
        
        return target_power / (total_power + 1e-12)
    
    def compute_phase_amplitude_coupling(self, eeg: np.ndarray, 
                                        phase_band: str = 'theta',
                                        amplitude_band: str = 'gamma_low') -> float:
        """
        ...
        # Handle empty bins
        mean_amp_per_bin[mean_amp_per_bin == 0] = np.mean(mean_amp_per_bin[mean_amp_per_bin > 0])
...
class APGIStatisticalValidation:
    ...
    def permutation_test(self, observed_z: float, modality: str,
                        null_distribution: Optional[np.ndarray] = None) -> Dict:
        """
    ...
    def bootstrap_confidence_interval(self, data: np.ndarray, 
        ...
        for _ in range(n_bootstrap):
            ...
            # Compute z-score
            z = (np.mean(sample) - self.normalizer.norms[modality]['mean']) / \
    ...
    def cross_modal_consistency_check(self, z_scores: Dict[str, float],
                                     expected_correlations: Dict[Tuple[str, str], float]) -> Dict:
        """
...
class APGITemporalDynamics:
    ...
    def __init__(self, normalizer, window_size: float = 2.0, 
                 step_size: float = 0.5, fs: int = 250):
        """
        Args:
    ...
    def protocol_1_validate_window_length(self, 
                                          signal: np.ndarray,
                                          modality: str,
                                          window_range: Tuple[float, float] = (0.5, 5.0),
                                          n_windows_test: int = 10,
                                          criterion: str = 'stability') -> Dict[str, float]:
        """
    ...
    def optimize_window_for_apgi(self, 
                                 multimodal_data: Dict[str, np.ndarray],
                                 primary_modality: str = 'eeg') -> Dict[str, Any]:
        """
        ...
        secondary_results = {}
        for modality, signal in multimodal_data.items():
            if modality != primary_modality:
                try:
                    secondary_results[modality] = self.protocol_1_validate_window_length(
                        signal, modality, window_range=(0.5, 4.0), n_windows_test=10
                    )
                except Exception as e:
        ...
        recommendations = {
            ...
            'secondary_windows': {k: v['optimal_window_sec'] for k, v in secondary_results.items() if v['optimal_window_sec'] is not None},
        ...
        print(f"   Recommended window: {recommendations['recommended_window_sec']:.2f}s ({recommendations['recommended_window_samples']} samples)")
        
        return recommendations
        
    def compute_time_varying_precision(self, eeg: np.ndarray, 
                                       band: str = 'gamma_low') -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def compute_threshold_trajectory(self, pupil: np.ndarray, 
                                    alpha_power: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def detect_ignition_events(self, precision_z: np.ndarray, 
    ...
    def phase_locking_value(self, signal1: np.ndarray, signal2: np.ndarray,
...
class EnhancedClinicalInterpreter:
    ...
    def generate_report(self, z_scores: Dict[str, float], 
    ...
    def differential_diagnosis(self, apgi_params: Dict[str, float],
    ...
    def treatment_recommendations(self, diagnosis: str, 
                                 apgi_params: Dict[str, float]) -> List[str]:
        """
...
class APGIQualityControl:
    ...
    @staticmethod
    def validate_measurement(modality: str, value: float, 
                           normative_range: Optional[Tuple[float, float]] = None) -> bool:
        """
...
def align_modalities(timestamps_dict: Dict[str, Tuple[np.ndarray, float]], 
                    target_fs: float = 100) -> Dict[str, np.ndarray]:
    """
...
def decorrelate_modalities(z_scores: Dict[str, float], 
...
def compute_HEP_zscore(ecg: np.ndarray, eeg: np.ndarray, normalizer: APGINormalizer, 
                      fs: int = 250, ch_names: Optional[List[str]] = None) -> float:
    """
    ...
    def detect_r_peaks(ecg_signal, sampling_rate):
        ...
        peaks, _ = find_peaks(ecg_signal, height=height_threshold, 
    ...
    if len(r_peaks) == 0:
        ...
        warnings.warn(f"Insufficient R-peaks for reliable HEP: {len(r_peaks)} "
    ...
    if epochs.shape[0] == 0:
        warnings.warn("No valid EEG epochs extracted - possible timing mismatch "
...
def compute_threshold_composite(pupil_mm: float, alpha_power: float, 
                               normalizer: APGINormalizer) -> float:
    """
...
def compute_surprise_zscore(erp_waveform: np.ndarray, normalizer: APGINormalizer, 
                           fs: int = 250) -> float:
    """
...
class APGIMultiModalNetwork(nn.Module):
    ...
    def __init__(self, 
        ...
        self.peripheral_encoder = nn.Sequential(
            nn.Linear(n_peripheral_features, 32),
            nn.ReLU(),
            nn.Linear(32, 64)
        )
    ...
    def forward(self, eeg_z: torch.Tensor, fmri_z: torch.Tensor, 
                peripheral_z: torch.Tensor) -> Dict[str, torch.Tensor]:
        # Encode modalities
...
class RobustAPGINetwork(nn.Module):
    ...
    def impute_missing(self, eeg_z: torch.Tensor, fmri_z: torch.Tensor, 
                      peripheral_z: torch.Tensor, 
                      modality_mask: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """Impute missing modalities using available ones"""
        batch_size = eeg_z.shape[0]
        ...
        if torch.any(missing_peripheral):
            ...
            peripheral_imputed[missing_peripheral] = self.imputation_networks['peripheral'](other_features[missing_peripheral])
        
        return eeg_imputed, fmri_imputed, peripheral_imputed
    
    def forward(self, eeg_z: torch.Tensor, fmri_z: torch.Tensor, 
                peripheral_z: torch.Tensor, 
                modality_mask: torch.Tensor) -> Dict[str, torch.Tensor]:
...
class APGIDataset(Dataset):
    ...
    def _validate_raw_data(self, raw_data: Dict[str, np.ndarray]):
        ...
        for key in ['eeg', 'fmri', 'pupil', 'scr', 'hr']:
            if len(raw_data[key]) != n_samples:
                raise ValueError(f"All data arrays must have same length. {key} has {len(raw_data[key])}, expected {n_samples}")
    ...
    def set_modality_mask(self, mask: np.ndarray):
        ...
        assert mask.shape == (len(self), 3), f"Invalid mask shape {mask.shape}, expected ({len(self)}, 3)"
...
def train_apgi_model(model: nn.Module, 
...
def rolling_zscore(signal: np.ndarray, window: int = 1000, 
                  min_periods: int = 100, robust: bool = False) -> np.ndarray:
    """
...
class APGIBatchProcessor:
    ...
    def _compute_band_power(self, signal: np.ndarray, freq_range: Tuple[float, float]) -> float:
        """Compute power in frequency band"""
        f, Pxx = signal.welch(signal, fs=self.config.get('fs', 250))
        mask = (f >= freq_range[0]) & (f <= freq_range[1])
        return np.mean(Pxx[mask])
    
    def _compute_apgi_parameters(self, z_scores: Dict[str, float], raw_signals: Dict[str, np.ndarray]) -> Dict[str, float]:
...
if __name__ == "__main__":
    ...
    if details['optimal_window_sec']:
        print(f"\n📈 VALIDATION METRICS:")
        print(f"   Stability criterion: {details['optimal_criterion']}")
        print(f"   Optimal score: {details.get('optimal_score', 0):.3f}")
        print(f"   Window range tested: {details['window_sizes_sec'][0]:.1f}s - {details['window_sizes_sec'][-1]:.1f}s")
    ...
    apgi_params = {
        ...
        'θ_t': compute_threshold_composite(
            new_subject['pupil_diameter'], 
            new_subject['alpha_power'],
            normalizer
        ),
        'S_t': compute_surprise_zscore(
    ...
    for modality, z in z_scores.items():
        ...
        print(f"  {modality}: z={z_scalar:.3f}, p={result['p_value']:.4f}, "
    ...
    print(f"  Interpretation: {'High probability of conscious access' if ignition_prob > 0.5 else 'Low probability of conscious access'}")
    ...
    correlation_matrix = np.array([[1.0, 0.3, 0.2],
                                   [0.3, 1.0, 0.1],
                                   [0.2, 0.1, 1.0]])  # Example correlations
    ...
    model = RobustAPGINetwork(
        n_eeg_features=64,
        n_fmri_features=1000,
        n_peripheral_features=3
    )
    ...
    for i in range(15):
        ...
        if result:
            print(f"  Sample {i+1}: z_pop={result['z_population']:.3f}, "
