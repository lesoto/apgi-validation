...
class APGITuringMachine:
    ...
    def is_interoceptive(signal: SignalModality) -> bool:
        ...
        return signal in [SignalModality.HEARTBEAT, SignalModality.RESPIRATION, 
                         SignalModality.PAIN, SignalModality.HUNGER]
    
    @staticmethod
    def is_exteroceptive(signal: SignalModality) -> bool:
        """Check if signal is exteroceptive"""
        return signal in [SignalModality.VISUAL, SignalModality.AUDITORY, 
    ...
    def compute_weighted_prediction_error(self, 
                                         signal: SignalModality,
                                         raw_error: float) -> float:
        """
    ...
    def transition_surprise_accumulation(self, 
                                        signal: SignalModality,
        ...
        if (self.state.S_continuous >= current_threshold and
            workspace_level >= 0.6 and  # Sufficient evidence
            self.state.surprise == SurpriseLevel.HIGH and
    ...
    def _advance_workspace(self) -> None:
        ...
        level_sequence = [
            WorkspaceLevel.W0, WorkspaceLevel.W1, WorkspaceLevel.W2,
            WorkspaceLevel.W3, WorkspaceLevel.W4, WorkspaceLevel.W5
        ]
    ...
    def _compute_somatic_marker(self) -> float:
        ...
        valence_probs = [(0.6, 1.0), (0.3, 0.0), (0.1, -1.0)]  # Positive, Neutral, Negative
        valence = np.random.choice([v for _, v in valence_probs],
                                  p=[p for p, _ in valence_probs])
    ...
    def integrate_continuous_dynamics(self, dt: float) -> None:
        ...
        dTheta = ((current_theta_target - self.state.theta_continuous) / 
                  self.params.tau_theta) * dt
...
class APGIVisualizer:
    ...
    @staticmethod
    def plot_simulation(machine: APGITuringMachine, 
                       title: str = "APGI Turing Machine Simulation"):
        """
        ...
        ax0 = axes[0]
        ax0.plot(df['time'], df['S'], label='$S_t$ (Surprise)', 
                color='blue', linewidth=1.5)
        ax0.plot(df['time'], df['theta'], label=r'$\theta_t$ (Threshold)', 
                color='orange', linestyle='--', linewidth=1.5)
        
        # Mark ignitions
        ignited = df[df['ignition'] == 'IGNITED']
        if len(ignited) > 0:
            ax0.scatter(ignited['time'], ignited['S'], 
        ...
        ln1 = ax1.plot(df['time'], df['workspace'], 
                      label='Workspace Level', color='green', linewidth=1.5)
        ln2 = ax1_twin.plot(df['time'], df['energy'], 
                           label='Energy Consumed', color='brown', 
        ...
        neuromod_map = {'DOPAMINE': 0, 'NOREPINEPHRINE': 1, 
                       'ACETYLCHOLINE': 2, '5HT': 3}
        df['neuromod_numeric'] = df['neuromod'].map(neuromod_map)
        
        ax2.step(df['time'], df['neuromod_numeric'], 
                where='post', label='Neuromodulator', linewidth=1.5)
        ...
        ax3.plot(df['time'], df['ignitions'], 
                label='Cumulative Ignitions', color='purple', 
    ...
    def plot_event_timeline(machine: APGITuringMachine):
        ...
        colors = {'IGNITION': 'red', 'GLOBAL_BROADCAST': 'blue', 
        ...
        for i, (event_type, times) in enumerate(event_types.items()):
            ax.scatter(times, [i] * len(times), 
                      ...
                      s=100, label=event_type, alpha=0.7)
...
def run_baseline_simulation(duration: float = 120.0, dt: float = 0.05):
    ...
    # Signal modalities
    signals = [SignalModality.VISUAL if t < steps // 2 else SignalModality.HEARTBEAT 
...
def run_anxiety_comparison(duration: float = 60.0):
    ...
    print("Results:")
    print(f"  Neutral - Ignitions: {machine_neutral.state.total_ignitions}, "
          f"Energy: {machine_neutral.state.energy_consumed:.1f}")
    print(f"  Anxiety - Ignitions: {machine_anxiety.state.total_ignitions}, "
...
if __name__ == "__main__":
    ...
    fig3 = APGIVisualizer.plot_simulation(machine_neutral, 
                                         title="APGI - Neutral Individual")
    fig4 = APGIVisualizer.plot_simulation(machine_anxiety,
                                         title="APGI - High Anxiety Individual")
