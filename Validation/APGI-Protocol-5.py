...
class AgentGenome:
    ...
    def mutate(self, mutation_rate: float = 0.1) -> 'AgentGenome':
        ...
        if np.random.random() < mutation_rate:
            mutated.beta = np.clip(
                mutated.beta * np.random.uniform(0.8, 1.2), 0.3, 2.5
            )
        ...
        if np.random.random() < mutation_rate:
            mutated.hidden_dim = np.clip(
                int(mutated.hidden_dim * np.random.choice([0.5, 1.5, 2.0])),
                8, 128
            )
...
class EvolvableAgent:
    ...
    def compute_surprise(
        ...
        if self.genome.has_intero_weighting:
            if self.genome.has_precision_weighting:
                intero_component = (self.genome.beta * self.Pi_i * 
                                  abs(internal_signal))
            else:
    ...
    def decide(
        ...
        if self.genome.has_threshold:
            ...
            P_ignition = 1.0 / (1.0 + np.exp(
                -self.genome.alpha * (S_t - self.genome.theta_0)
            ))
    ...
    def update(
        ...
        if self.genome.has_somatic_markers:
            ...
            self.somatic_markers[state_hash][action] += (
...
class EvolutionaryOptimizer:
    ...
    def initialize_population(self):
        ...
        self.population = [
            AgentGenome.random() for _ in range(self.pop_size)
        ]
    ...
    def compute_architecture_frequencies(self) -> Dict[str, float]:
        ...
        return {
            'has_threshold': np.mean([
                g.has_threshold for g in self.population
            ]),
...
def plot_evolutionary_results(
    history: Dict,
    save_path: str = 'protocol5_evolution_results.png'
):
    ...
    for idx, (trait, label, color) in enumerate(traits):
        ...
        ax.axhline(y=0.9, color='red', linestyle='--', linewidth=1.5, alpha=0.5,
    ...
    bars = ax_sel.bar(trait_names, sel_values, color=colors_sel, alpha=0.7,
                      edgecolor='black', linewidth=1.5)
    
    ax_sel.axhline(y=0, color='black', linestyle='-', linewidth=1)
    ax_sel.axhline(y=0.02, color='red', linestyle='--', linewidth=1.5,
    ...
    for bar, val in zip(bars, sel_values):
        ...
        ax_sel.text(bar.get_x() + bar.get_width()/2., height,
    ...
    if fix_values:
        ax_fix.barh(fix_labels, fix_values, color=colors_sel[:len(fix_values)],
    ...
    ax_order.text(0.1, 0.5, order_text, fontsize=10, family='monospace',
...
def main():
    ...
    plot_evolutionary_results(
        history,
        save_path='protocol5_evolution_results.png'
    )
