...
class ComprehensivePhaseTransitionAnalysis:
    ...
    def analyze_simulation(
        ...
        if len(S) > 20:
            te_S_to_B = self.info_analyzer.compute_transfer_entropy(
                S, B, lag=1
            )
    ...
    def run_monte_carlo(
        ...
        print(f"\n{'='*80}")
        print(f"RUNNING MONTE CARLO PHASE TRANSITION ANALYSIS")
        print(f"{'='*80}")
        ...
        if save_results:
            ...
            print(f"   Results saved to: protocol4_monte_carlo_results.csv")
...
def plot_phase_transition_results(
    ...
    if example_history is not None:
        ...
        for idx in ignition_events:
            if idx < len(time):
                ax1.axvline(time[idx], color='orange', linestyle=':', 
                           linewidth=1.5, alpha=0.6)
        
        ax1.set_xlabel('Time (s)', fontsize=12, fontweight='bold')
        ax1.set_ylabel('Magnitude', fontsize=12, fontweight='bold')
        ax1.set_title('Example APGI Dynamics: Surprise Accumulation and Ignition',
        ...
        if len(ignition_events) > 0:
            ...
            ax1_inset.plot(time[zoom_start:zoom_end], S[zoom_start:zoom_end],
                          'b-', linewidth=2)
            ax1_inset.plot(time[zoom_start:zoom_end], theta[zoom_start:zoom_end],
                          'r--', linewidth=2)
            ax1_inset.axvline(time[zoom_idx], color='orange', linestyle=':',
                            linewidth=2)
    ...
    if len(phi_at_ignition) > 0 and len(phi_baseline) > 0:
        ...
        ax2.text(0.5, 0.95, f'Ratio: {ratio:.2f}×',
    ...
    if 'te_S_to_B' in results_df.columns:
        ...
        ax3.hist(te_values, bins=30, density=True, alpha=0.7,
                color='purple', edgecolor='black')
        
        mean_te = te_values.mean()
        ax3.axvline(mean_te, color='red', linestyle='--', linewidth=2,
        ...
        ax3.set_title('Information Flow: Surprise → Ignition',
    ...
    if 'mi_S_theta' in results_df.columns:
        ...
        ax4.hist(mi_values, bins=30, density=True, alpha=0.7,
                color='teal', edgecolor='black')
        
        mean_mi = mi_values.mean()
        ax4.axvline(mean_mi, color='red', linestyle='--', linewidth=2,
        ...
        ax4.set_title('Statistical Dependence',
                     fontsize=12, fontweight='bold')
    ...
    if 'discontinuity_cohens_d' in results_df.columns:
        ...
        ax6.hist(disc_d, bins=30, density=True, alpha=0.7,
                color='orange', edgecolor='black')
        
        mean_d = disc_d.mean()
        ax6.axvline(mean_d, color='red', linestyle='--', linewidth=2,
                   label=f"Mean d: {mean_d:.2f}")
        ax6.axvline(0.8, color='green', linestyle=':', linewidth=2,
        ...
        ax6.set_title('P4a: Discontinuity at Ignition',
                     fontsize=12, fontweight='bold')
    ...
    ax7.hist(susc_ratio, bins=30, density=True, alpha=0.7,
            color='red', edgecolor='black')
    
    mean_susc = susc_ratio.mean()
    ax7.axvline(mean_susc, color='darkred', linestyle='--', linewidth=2,
               label=f'Mean: {mean_susc:.2f}')
    ax7.axvline(2.0, color='green', linestyle=':', linewidth=2,
    ...
    ax7.set_title('P4b: Diverging Susceptibility',
                 fontsize=12, fontweight='bold')
    ...
    ax8.hist(crit_slow, bins=30, density=True, alpha=0.7,
            color='blue', edgecolor='black')
    
    mean_cs = crit_slow.mean()
    ax8.axvline(mean_cs, color='darkblue', linestyle='--', linewidth=2,
               label=f'Mean: {mean_cs:.2f}')
    ax8.axvline(1.5, color='green', linestyle=':', linewidth=2,
               label='Prediction: >1.5')
    
    ax8.set_xlabel('Critical Slowing Ratio', fontsize=11, fontweight='bold')
    ax8.set_ylabel('Density', fontsize=11, fontweight='bold')
    ax8.set_title('P4c: Critical Slowing Down',
                 fontsize=12, fontweight='bold')
    ...
    bp = ax9.boxplot(data, positions=positions, widths=0.6,
                    patch_artist=True, showmeans=True)
    
    for patch, color in zip(bp['boxes'], ['lightblue', 'salmon']):
        patch.set_facecolor(color)
    
    ax9.axhline(0.5, color='black', linestyle='--', linewidth=1.5,
               label='Random walk (H=0.5)')
    ax9.axhline(0.6, color='green', linestyle=':', linewidth=2,
               label='Prediction: H>0.6')
    
    ax9.set_xticks(positions)
    ax9.set_xticklabels(['Far', 'Near Threshold'])
    ax9.set_ylabel('Hurst Exponent', fontsize=11, fontweight='bold')
    ax9.set_title('P4e: Long-Range Correlations',
                 fontsize=12, fontweight='bold')
    ...
    if len(corr_data) > 10:
        ...
        sns.heatmap(corr_matrix, annot=True, fmt='.2f', cmap='coolwarm',
                   center=0, vmin=-1, vmax=1, square=True, ax=ax11,
                   xticklabels=measure_labels, yticklabels=measure_labels,
                   cbar_kws={'label': 'Correlation'})
        
        ax11.set_title('Phase Transition Measure Correlations',
                      fontsize=12, fontweight='bold')
...
def main():
    ...
    plot_phase_transition_results(
        ...
        save_path='protocol4_results.png'
    )
