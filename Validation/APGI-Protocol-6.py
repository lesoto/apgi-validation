...
class APGIInspiredNetwork(nn.Module):
    ...
    def __init__(self, config: Dict):
        ...
        self.surprise_rnn = nn.GRUCell(
            input_size=2, hidden_size=16  # Precision-weighted errors
        )
...
def visualize_attention_patterns(model, test_batch, return_attention=False):
    ...
    with torch.no_grad():
        ...
        axes[0, 0].imshow(
            attention_weights["extero"].cpu().numpy(), aspect="auto", cmap="hot"
        )
        ...
        axes[0, 1].imshow(
            attention_weights["intero"].cpu().numpy(), aspect="auto", cmap="hot"
        )
