...
class PsychometricCurve:
    ...
    def fit_curve(
        ...
        def negative_log_likelihood(params):
            ...
            ll = np.sum(
                n_correct * np.log(p_pred) +
                (n_trials - n_correct) * np.log(1 - p_pred)
            )
    ...
    def compare_curves(
        ...
        # Threshold shift
        threshold_shift = (intervention_params['threshold'] - 
        ...
        # Slope change
        slope_change = (intervention_params['slope'] - 
                       baseline_params['slope'])
        
        slope_change_se = np.sqrt(
            baseline_params['slope_se']**2 +
            intervention_params['slope_se']**2
        )
    ...
    def simulate_crossover_study(
        ...
        for subj_id in range(n_subjects):
            ...
            for session, condition in enumerate(order):
                ...
                time_point = 30.0
                effect_magnitude = effect.compute_time_course(
                    np.array([time_point])
                )[0]
...
def plot_intervention_results(
    ...
    save_path: str = 'protocol3_intervention_results.png'
):
    ...
    intervention_grouped['p_seen'] = intervention_grouped['n_seen'] / intervention_grouped['n_trials']
    ...
    # Plot data points
    ax1.scatter(baseline_grouped.index, baseline_grouped['p_seen'],
               s=100, alpha=0.6, color='blue', label='Baseline', zorder=3)
    ax1.scatter(intervention_grouped.index, intervention_grouped['p_seen'],
               s=100, alpha=0.6, color='red', label='Intervention', zorder=3)
    ...
    y_baseline = logistic(x_fine, baseline_params['threshold'],
                         baseline_params['slope'], baseline_params['lapse'])
    y_intervention = logistic(x_fine, intervention_params['threshold'],
                             intervention_params['slope'], intervention_params['lapse'])
    
    ax1.plot(x_fine, y_baseline, 'b-', linewidth=2.5, alpha=0.8)
    ax1.plot(x_fine, y_intervention, 'r-', linewidth=2.5, alpha=0.8)
    
    # Threshold markers
    ax1.axvline(baseline_params['threshold'], color='blue', linestyle='--',
               linewidth=2, alpha=0.6)
    ax1.axvline(intervention_params['threshold'], color='red', linestyle='--',
               linewidth=2, alpha=0.6)
    
    ax1.set_xlabel('Stimulus Intensity', fontsize=13, fontweight='bold')
    ax1.set_ylabel('P(Seen)', fontsize=13, fontweight='bold')
    ax1.set_title(f'Psychometric Functions - {intervention_name}',
    ...
    threshold_shift_se = np.sqrt(baseline_params['threshold_se']**2 +
                                intervention_params['threshold_se']**2)
    
    ax2.bar(['Baseline', 'Intervention'],
           [baseline_params['threshold'], intervention_params['threshold']],
           color=['blue', 'red'], alpha=0.7, edgecolor='black', linewidth=2)
    
    ax2.errorbar(['Baseline', 'Intervention'],
                [baseline_params['threshold'], intervention_params['threshold']],
    ...
    ax2.set_title(f'Threshold Shift: {threshold_shift:.3f}±{threshold_shift_se:.3f}',
    ...
    subject_thresholds = results_df.groupby(['subject_id', 'condition']).apply(
        lambda x: psychometric.fit_curve(
            x['stimulus_level'].values,
            x['n_trials'].values,
            x['n_seen'].values
        )['threshold'] if len(x) > 0 else np.nan
    ).unstack()
    ...
    for i, subj in enumerate(subject_ids):
        ax3.plot([0, 1], [baseline_thresholds[i], intervention_thresholds[i]],
                'o-', color='gray', alpha=0.4, linewidth=1, markersize=6)
    
    # Mean effect
    ax3.plot([0, 1], [np.nanmean(baseline_thresholds), np.nanmean(intervention_thresholds)],
            'o-', color='red', linewidth=3, markersize=12, label='Mean Effect')
    ...
    ax4.hist(individual_effects, bins=15, density=True, alpha=0.6,
    ...
    ax4.plot(x_dist, stats.norm.pdf(x_dist, mu_effect, sigma_effect),
            'r-', linewidth=2.5, label=f'N({mu_effect:.3f}, {sigma_effect:.3f})')
    ...
    # Compute statistics
    t_stat, p_value = stats.ttest_rel(baseline_thresholds, intervention_thresholds)
    cohens_d = (np.nanmean(intervention_thresholds) - np.nanmean(baseline_thresholds)) / \
    ...
    ax6.axvline(len(individual_effects), color='green', linestyle='--',
               linewidth=2, label=f'Current N={len(individual_effects)}')
...
def main():
    ...
    for name, intervention in interventions.items():
        print(f"  - {name}: {intervention.target_parameter} "
    ...
    dlpfc_results = {
        ...
        'threshold_se': comparison['threshold_shift_se']
    }
    ...
    plot_intervention_results(
        dlpfc_data,
        'dlPFC TMS',
        save_path='protocol3_dlpfc_results.png'
    )
    
    plot_intervention_results(
        propranolol_data,
        'Propranolol',
        save_path='protocol3_propranolol_results.png'
    )
    ...
    # Save data
    dlpfc_data.to_csv('protocol3_dlpfc_data.csv', index=False)
    propranolol_data.to_csv('protocol3_propranolol_data.csv', index=False)
