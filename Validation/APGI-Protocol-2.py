...
class SyntheticConsciousnessDataGenerator:
    ...
    def generate_melloni_style_data(
        ...
        for trial in range(n_trials):
            ...
            S_t = (Pi_e * stimulus_strength[trial] * prediction_error[trial] +
                   beta_true[subj] * Pi_i_true[subj] * np.abs(eps_i))
    ...
    def generate_canales_johnson_style_data(
        ...
        for trial in range(n_trials):
            ...
            # APGI
            S_t = (stimulus_strength[trial] * prediction_error[trial] +
                   beta_true[subj] * Pi_i_true[subj] * np.abs(eps_i))
...
class APGIGenerativeModel:
    ...
    def build_model(self, data: ConsciousnessDataset) -> pm.Model:
        ...
        with pm.Model() as model:
            ...
            # Threshold
            theta_0_offset = pm.Normal('theta_0_offset', mu=0, sigma=1, 
                                      shape=n_subjects)
            theta_0 = pm.Deterministic('theta_0', 
                                      mu_theta + sigma_theta * theta_0_offset)
            theta_0_bounded = pm.math.clip(theta_0, 0.2, 0.9)
            
            # Interoceptive precision
            Pi_i_offset = pm.Normal('Pi_i_offset', mu=0, sigma=1, 
                                   shape=n_subjects)
            Pi_i = pm.Deterministic('Pi_i', 
                                   mu_Pi_i + sigma_Pi_i * Pi_i_offset)
            ...
            intero_contrib = (beta_bounded[subj_idx] * 
                             Pi_i_bounded[subj_idx] * 
                             pm.math.abs(eps_i))
            
            S_t = pm.Deterministic('S_t', extero_contrib + intero_contrib)
            
            # Dynamic threshold (baseline + trial noise)
            theta_noise = pm.Normal('theta_noise', mu=0, sigma=0.1, 
                                   shape=n_trials)
            theta_t = pm.Deterministic('theta_t', 
                                      theta_0_bounded[subj_idx] + theta_noise)
            ...
            pm.Bernoulli('y_report', p=P_ignition, 
                        observed=data_dict['conscious_report'])
            ...
            if 'reaction_time' in data_dict:
                ...
                pm.Wald('y_RT', 
                       mu=RT_mu, 
                       lam=50000,
                       observed=data_dict['reaction_time'])
            ...
            if 'HEP_amplitude' in data_dict:
                ...
                pm.Normal('y_HEP', 
                         mu=HEP_pred, 
                         sigma=1.0,
                         observed=data_dict['HEP_amplitude'])
...
class StandardSDTModel:
    ...
    def build_model(self, data: ConsciousnessDataset) -> pm.Model:
        ...
        with pm.Model() as model:
            ...
            d_prime = pm.Normal('d_prime', mu=1.5, sigma=1.0, 
                               shape=n_subjects)
            criterion = pm.Normal('criterion', mu=0, sigma=1.0, 
                                 shape=n_subjects)
            ...
            # Likelihood
            pm.Bernoulli('y_report', p=P_report, 
                        observed=data_dict['conscious_report'])
    ...
    def build_model(self, data: ConsciousnessDataset) -> pm.Model:
        ...
        with pm.Model() as model:
            ...
            theta = pm.Deterministic('theta', 
                                    mu_theta + sigma_theta * theta_offset)
            ...
            S_t = (data_dict['stimulus_strength'] * 
            ...
            if 'P3b_amplitude' in data_dict:
                seen_mask = data_dict['conscious_report'] == 1
                P3b_pred = 2.0 + 8.0 * pm.math.maximum(
                    S_t - theta_bounded[subj_idx], 0
                )
...
class ContinuousIntegrationModel:
    ...
    def build_model(self, data: ConsciousnessDataset) -> pm.Model:
        ...
        with pm.Model() as model:
            ...
            gain_offset = pm.Normal('gain_offset', mu=0, sigma=1, 
                                   shape=n_subjects)
            gain = pm.Deterministic('gain', 
                                   mu_gain + sigma_gain * gain_offset)
            gain_bounded = pm.math.clip(gain, 0.5, 4.0)
            
            subj_idx = data_dict['subject_idx']
            
            # Continuous accumulation
            evidence = (data_dict['stimulus_strength'] * 
                       data_dict['prediction_error'] * 
        ...
        for fold_idx, (train_subjects, test_subjects) in enumerate(kf.split(unique_subjects)):
            ...
            for name, model_class in self.models.items():
                ...
                try:
                    ...
                    with model:
                        trace = pm.sample(
                            1000, tune=500, chains=2, cores=2,
                            progressbar=False,
                            return_inferencedata=True
                        )
...
class FalsificationChecker:
    ...
    def __init__(self):
        self.criteria = {
            ...
            'F2.5': {
                'description': 'BF for APGI vs GWT < 3',
                'threshold': 3.0
            }
        }
...
def plot_model_comparison_results(
    comparison_df: pd.DataFrame,
    save_path: str = 'protocol2_model_comparison.png'
):
    ...
    ax1.barh(y_pos, -loos, xerr=loo_ses, color=bar_colors, alpha=0.7,
    ...
    ax1.axvline(x=-comparison_df['loo'].min(), color='red', linestyle='--',
    ...
    ax2.barh(y_pos, delta_loos, color=bar_colors, alpha=0.7,
            edgecolor='black', linewidth=1.5)
    ...
    ax3.barh(y_pos, p_loos, color=bar_colors, alpha=0.7,
            edgecolor='black', linewidth=1.5)
    ...
    ax4.barh(y_pos, -waics, xerr=waic_ses, color=bar_colors, alpha=0.7,
            edgecolor='black', linewidth=1.5)
    ...
    if 'BF_vs_apgi' in comparison_df.columns:
        ...
        # Plot on log scale
        ax5.barh(y_pos, np.log10(bfs), color=bar_colors, alpha=0.7,
                edgecolor='black', linewidth=1.5)
        ...
        ax5.axvline(x=np.log10(3), color='orange', linestyle='--', linewidth=2,
                   label='Weak Evidence (BF=3)')
        ax5.axvline(x=np.log10(10), color='green', linestyle='--', linewidth=2,
...
def plot_posterior_distributions(
    trace,
    save_path: str = 'protocol2_posteriors.png'
):
...
def main():
    ...
    print(f"✅ {canales_data.name}: {canales_data.n_subjects} subjects, "
    ...
    plot_model_comparison_results(
        melloni_comparison_df,
        save_path='protocol2_melloni_comparison.png'
    )
    
    plot_model_comparison_results(
        canales_comparison_df,
        save_path='protocol2_canales_comparison.png'
    )
    ...
    if apgi_trace_melloni is not None:
        plot_posterior_distributions(
            apgi_trace_melloni,
            save_path='protocol2_melloni_posteriors.png'
        )
    
    if apgi_trace_canales is not None:
        plot_posterior_distributions(
            apgi_trace_canales,
            save_path='protocol2_canales_posteriors.png'
        )
    ...
    results_summary = {
        ...
        'melloni': {
            ...
            'ppc': {k: {kk: float(vv) if isinstance(vv, (np.floating, float)) else vv 
