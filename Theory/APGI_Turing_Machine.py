...
def run_baseline_simulation(duration: float = 120.0, dt: float = 0.05):
    ...
    print(f"Parameters: Π_e={params.Pi_e:.2f}, Π_i={params.Pi_i:.2f}, β_som={params.beta:.2f}")
    print()
    ...
    # Signal modalities
    signals = [SignalModality.VISUAL if t < steps // 2 else SignalModality.HEARTBEAT for t in range(steps)]
...
def run_anxiety_comparison(duration: float = 60.0):
    ...
    print(f"Anxiety β_som={params_anxiety.beta:.3f}, κ_NE={params_anxiety.kappa_NE:.3f}")
    print()
    ...
    eps_threat[threat_start:threat_end] += np.random.normal(2.5, 0.3, threat_end - threat_start)

    # Add second stronger threat event to amplify individual differences
    threat2_start = int(40.0 / dt)
    threat2_end = int(45.0 / dt)
    eps_threat[threat2_start:threat2_end] += np.random.normal(3.0, 0.3, threat2_end - threat2_start)

    contexts = [TaskContext.REST] * steps
    contexts[threat_start:threat_end] = [TaskContext.THREAT] * (threat_end - threat_start)
    contexts[threat2_start:threat2_end] = [TaskContext.THREAT] * (threat2_end - threat2_start)
