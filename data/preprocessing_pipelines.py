...
class EEGPreprocessor:
    ...
    def preprocess_eeg(self, df: pd.DataFrame, eeg_columns: List[str] = None) -> pd.DataFrame:
    ...
    def _apply_bandpass_filter(self, signal_data: pd.Series) -> pd.Series:
        ...
        if fs is None:
            self.preprocessing_log.append("Could not estimate sampling rate, skipping bandpass filter")
            return signal_data
        ...
        try:
            ...
            self.preprocessing_log.append(f"Applied bandpass filter ({self.config.eeg_bandpass_low}-{self.config.eeg_bandpass_high} Hz)")
...
class PupilPreprocessor:
    ...
    def preprocess_pupil(self, df: pd.DataFrame, pupil_column: str = 'pupil_diameter') -> pd.DataFrame:
        ...
        if self.config.pupil_blink_detection:
            df_processed[pupil_column] = self._detect_and_handle_blinks(df_processed[pupil_column])
        
        # Step 2: Smooth the signal
        df_processed[pupil_column] = self._smooth_pupil_signal(df_processed[pupil_column])
        
        # Step 3: Normalize pupil diameter
        df_processed[pupil_column] = self._normalize_pupil_diameter(df_processed[pupil_column])
...
class EDAPreprocessor:
    ...
    def _extract_phasic_tonic(self, df: pd.DataFrame, eda_column: str) -> pd.DataFrame:
        ...
        try:
            ...
            tonic = eda_data.rolling(window=int(tonic_window * self._estimate_sampling_rate(eda_data) or 1000), 
            ...
            self.preprocessing_log.append(f"Error extracting phasic/tonic components: {e}")
            return df
...
class HeartRatePreprocessor:
    ...
    def preprocess_heart_rate(self, df: pd.DataFrame, hr_column: str = 'heart_rate') -> pd.DataFrame:
        ...
        df_processed[hr_column] = self._detect_and_handle_outliers(df_processed[hr_column])
        
        # Step 2: Interpolate missing values
        df_processed[hr_column] = df_processed[hr_column].interpolate(method=self.config.hr_interpolation_method)
...
class MultimodalPreprocessingPipeline:
    ...
    def run_complete_pipeline(self, input_file: Union[str, Path], 
                           output_dir: Union[str, Path] = "data/processed") -> Dict[str, Any]:
        ...
        self.pipeline_log.append(f"Validation complete - Quality score: {validation_report['data_quality'].get('overall_score', 'N/A')}")
        ...
        self.pipeline_log.append(f"Final quality score: {final_quality['overall_score']:.1f}")
        ...
        return {
            ...
            'initial_quality': validation_report['data_quality'].get('overall_score', 0),
            'final_quality': final_quality['overall_score'],
            'quality_improvement': final_quality['overall_score'] - validation_report['data_quality'].get('overall_score', 0),
    ...
    def _save_processing_report(self, initial_validation: Dict, final_quality: Dict, 
                              report_file: Path):
        ...
        report = {
            ...
            'pipeline_log': self.pipeline_log,
            'preprocessing_steps': self.preprocessor.preprocessing_steps,
            'quality_improvement': final_quality['overall_score'] - initial_validation['data_quality'].get('overall_score', 0)
        }
