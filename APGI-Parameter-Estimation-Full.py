        ...
        for subj_id in range(n_subjects):
            ...
            for trial in range(n_trials):
                ...
                conf = min(
                    ...
                    max(
                        1,
                        int(3 * abs(current_intensity - theta0) + 1 + np.random.normal(0, 0.5)),
                    ),
                )
...
def artifact_rejection_pipeline(data, method="faster"):
    ...
    for subj_id, subj_data in data.items():
        ...
        if method == "faster":
            ...
            artifact_mask = (
                (heps > mean_hep + 3 * std_hep) | (heps < 0) | (heps < mean_hep - 3 * std_hep)
            )
...
def validate_parameter_recovery(true_params, trace, n_subjects=100):
    ...
    for param in param_names:
        if param in recovered and param in true_params:
            ...
            if param in trace.posterior:
                ...
                coverage = np.mean(
                    (true_vals >= lower[: len(true_vals)]) & (true_vals <= upper[: len(true_vals)])
                )
            else:
                coverage = np.nan
    ...
    for param, threshold in critical_thresholds.items():
        if param in results:
            if results[param]["r"] < threshold:
                falsified = True
                failure_reasons.append(f"{param}: r = {results[param]['r']:.3f} < {threshold}")
    ...
    for param in ["theta0", "alpha", "beta_Pi_i"]:
        if param in results and results[param]["coverage"] < 0.90:
            falsified = True
            failure_reasons.append(f"{param}: coverage = {results[param]['coverage']:.3f} < 0.90")
...
def assess_predictive_validity(data, trace, n_simulations=1000):
    ...
    if "theta0" in trace.posterior and "alpha" in trace.posterior:
        n_samples = 100
        theta0_samples = np.random.choice(trace.posterior["theta0"].values.flatten(), n_samples)
        alpha_samples = np.random.choice(trace.posterior["alpha"].values.flatten(), n_samples)
